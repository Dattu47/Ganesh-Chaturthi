...
// Ensures 100% offline & static fallback reliability on GitHub / Cloudflare Pages
...
// 3. FETCH MEDIA (SUPABASE PRIMARY -> DIRECT REST -> STATIC FALLBACK)
async function fetchGalleryMedia() {
  ...
  if (items.length === 0 && SUPABASE_CONFIG.url && SUPABASE_CONFIG.anonKey) {
    try {
      const res = await fetch(`${SUPABASE_CONFIG.url}/rest/v1/gallery?published=eq.true&order=year.desc,id.desc`, {
        headers: {
  ...
  // Static Fallback
  if (items.length === 0) {
    items = STATIC_BACKUP_GALLERY.slice();
  }
...
window.handleGalleryImageError = function (imgEl) {
  ...
  if (!alreadyTried && fallbackSrc) {
    ...
    if (wrap) {
      wrap.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; padding:1rem; text-align:center; background:rgba(255,255,255,0.8); color:var(--text-muted, #777);">
          <span style="font-size:1.8rem; margin-bottom:0.25rem;">🕉️</span>
          <p style="font-size:0.85rem; margin:0;">ఈ చిత్రం ప్రస్తుతం అందుబాటులో లేదు.</p>
        </div>
      `;
    }
  }
...
window.downloadGalleryPhoto = async function (primaryUrl, fallbackUrl, filename, event) {
  if (event) {
...
async function initDedicatedGallery() {
  ...
  try {
    ...
    if (!items || items.length === 0) {
      if (loadingStatusBox) {
        loadingStatusBox.innerHTML = `<p>ఉత్సవ గ్యాలరీ చిత్రాలు త్వరలోనే ప్రచురించబడును.</p>`;
      }
      return;
    }
    ...
    if (availableYears.length === 0) {
      if (loadingStatusBox) {
        loadingStatusBox.innerHTML = `<p>గ్యాలరీ చిత్రాలు ప్రస్తుతం అందుబాటులో లేవు.</p>`;
      }
      return;
    }
    ...
    if (yearSelect) {
      yearSelect.innerHTML = `
        <option value="all" selected>🌟 అన్ని సంవత్సరాలు (All 21 Years) - అన్ని ఫోటోలు</option>
        ${availableYears
          .map((yr) => `<option value="${yr}">${yr} వినాయక చవితి ఉత్సవాలు</option>`)
          .join("")}
      `;
    ...
    if (yearPillsContainer) {
      yearPillsContainer.innerHTML = `
        <button type="button" class="year-pill-btn active" data-year="all">అన్ని సంవత్సరాలు (All)</button>
        ${availableYears
          .map((yr) => `<button type="button" class="year-pill-btn" data-year="${yr}">${yr}</button>`)
          .join("")}
      `;
    ...
    // Render ALL photos side by side year-wise by default!
    renderAllPhotosSideBySide("all");

  } catch (err) {
    console.error("Gallery initialization error:", err);
    if (loadingStatusBox) {
      loadingStatusBox.innerHTML = `<p>చిత్రాలు లోడ్ చేయడంలో అంతరాయం ఏర్పడింది. దయచేసి కాసేపటి తర్వాత మళ్ళీ ప్రయత్నించండి.</p>`;
    }
  }
}
...
function handleYearSelection(selectedYear) {
  ...
  renderAllPhotosSideBySide(selectedYear);
}
...
function renderAllPhotosSideBySide(filterYear) {
  ...
  if (loadingStatusBox) loadingStatusBox.style.display = "none";

  const isAll = (filterYear === "all" || !filterYear);
  const yearsToRender = isAll
    ? availableYears
    : [filterYear];
  ...
  if (allPhotos.length === 0 && allVideos.length === 0) {
    container.innerHTML = `
      <div class="no-media-box" style="text-align: center; padding: 3rem 1rem; background: rgba(255,255,255,0.9); border-radius: 12px; border: 1px dashed var(--gold);">
  ...
  let html = `
    <!-- Top Summary Banner -->
    <div class="active-year-banner" style="background: rgba(255,255,255,0.94); backdrop-filter: blur(8px); border-radius: 14px; padding: 1rem 1.4rem; margin-bottom: 2rem; border: 2px solid var(--gold, #d4af37); box-shadow: 0 4px 16px rgba(0,0,0,0.06); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
      <div class="active-year-title" style="margin: 0; font-size: 1.35rem; color: var(--maroon, #721214); font-weight: 800; display: flex; align-items: center; gap: 0.5rem;">
        <span>🌺</span>
        <span>${bannerTitle}</span>
      </div>
      <div style="display: flex; align-items: center; gap: 0.75rem;">
        ${!isAll ? `<button type="button" onclick="handleYearSelection('all')" class="year-pill-btn" style="background: #fdfaf2; border: 1.5px solid var(--gold); font-size: 0.88rem; padding: 0.4rem 0.9rem; cursor: pointer; border-radius: 50px; font-weight: 700; color: var(--maroon);">← అన్ని ఫోటోలు</button>` : ""}
  ...
  if (allPhotos.length > 0) {
    ...
    allPhotos.forEach((photo) => {
      ...
      const fallbackPath = getGalleryFallbackPath(yr, filename);
      ...
      html += `
        <div class="gallery-photo-card" data-index="${thisPhotoIndex}" style="background: rgba(255,255,255,0.96); backdrop-filter: blur(8px);">
          ...
          <div class="photo-details-body">
            ...
            <button 
              ...
              onclick="downloadGalleryPhoto('${photo.file_url}', '${fallbackPath}', '${dlFilename}', event)"
  ...
  if (allVideos.length > 0) {
    html += `
      <div style="margin-top: 3rem;">
        <div class="media-group-heading" style="color: var(--maroon, #721214);">
    ...
    allVideos.forEach((video) => {
      ...
      html += `
        <div class="gallery-video-card" style="background: rgba(255,255,255,0.95); backdrop-filter: blur(8px);">
          ...
          <div style="padding: 0.6rem 0.25rem 0.25rem 0.25rem;">
            <h4 style="font-size: 1rem; font-weight: 700; color: var(--maroon, #721214); margin: 0;">${vTitle}</h4>
          </div>
        </div>
      `;
    });
